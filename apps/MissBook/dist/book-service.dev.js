...
var BOOK_KEY = 'myBooks';
var bookService = (_bookService = {
  getBooks: getBooks,
  getById: getById,
  addReview: addReview
}, _defineProperty(_bookService, "getById", getById), _defineProperty(_bookService, "getBooksFromAPI", getBooksFromAPI), _defineProperty(_bookService, "addBook", addBook), _bookService);
...
function getBooksFromAPI(query) {
  return axios.get("https://www.googleapis.com/books/v1/volumes?printType=books&q=".concat(query)).then(function (res) {
    return res.data;
  }).then(function (books) {
    // saveToStorage(BOOK_KEY, books)
    return books;
...
function addBook(book) {
  var newBook = {
    ...
    thumbnail: book.volumeInfo.thumbnail ? book.volumeInfo.thumbnail : "http://coding-academy.org/books-photos/20.jpg",
  ...
  return Promise.resolve('book added');
} // {
//   "id": "OXeMG8wNskc",
